...
We are continuing to work on ARM support, primarily focused on LLMs and transformer models for server-grade systems like AWS Graviton and Ampere Currently, we have limited alpha support for CNN models on embedded systems, particularly those with dot product instructions (ARMv8.2+). ARM and MacOS has beta support. Feel free to *pip install deepsparse-nightyly* if you would like to try it out. We would like to hear your use cases and keep you in the
