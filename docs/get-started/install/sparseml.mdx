...
SparseML is Neural Magic's core toolkit for streamlining the optimization of deep learning models using cutting-edge sparsification methods (pruning, quantization, etc.).
...
<VersionInjector targetProduct="sparseml" prepend="v" currentTag="main" ignoreNightly={true}>
    ```bash
    pip install git+https://github.com/neuralmagic/sparseml.git@VERSION[dev]
    ```
</VersionInjector>
